...
const AdminPageCreate = () => {
    ...
    const handleSave = async () => {
        try {
            ...
            // Construct payload in the target schema
            const payload: any = {
                ...
                heroSection: {
                    ...
                    ctaButton: formData.heroSection?.ctaButton || undefined,
                    statsText: formData.heroSection?.statsText || undefined,
                ...
                aboutSection: {
                    ...
                    highlights: formData.aboutSection?.highlights || [],
                },

                servicesSection: formData.servicesSection || { title: "", subtitle: "", services: [] },

                challengesSection: {
                    title: formData.challengesSection?.title || "",
                    subtitle: formData.challengesSection?.subtitle || "",
                ...
                teamSection: formData.teamSection || { title: "", subtitle: "", members: [] },

                testimonialsSection: {
                    title: formData.testimonialsSection?.title || "",
                    subtitle: formData.testimonialsSection?.subtitle || "",
                    testimonials: Array.isArray(formData.testimonialsSection)
                        ? formData.testimonialsSection.map((t: any) => ({
                            name: t.name || "",
                ...
                faqSection: {
                    title: formData.faqSection?.title || "",
                    subtitle: formData.faqSection?.subtitle || "",
                    faqs: Array.isArray(formData.faqSection)
                        ? formData.faqSection.map((f: any, index: number) => ({
                            question: f.question || "",
                            answer: f.answer || "",
                ...
                partnersSection: formData.partnersSection || { title: "", partners: [] },
            };

            // Collect files with deterministic keys
            ...
            // Append JSON payload FIRST as a plain string so backend sees it in req.body
            form.append('createPagePayload', JSON.stringify(payload));

            // Also append critical top-level fields as fallback (some multer setups drop text fields)
            if (payload.pageName) form.append('pageName', payload.pageName);
            if (payload.pageSlug) form.append('pageSlug', payload.pageSlug);
            if (payload.seoPageTitle) form.append('seoPageTitle', payload.seoPageTitle);
            if (typeof payload.status !== 'undefined') form.append('status', String(payload.status));

            // Map files to backend-expected field names
            files.forEach(({ key, file }) => {
                if (key === 'heroSection.backgroundImage') {
                    ...
                    // Optional: send top image if backend starts handling it later
                    form.append('topImage', file);
                } else {
                    // Fallback to original key if no mapping provided
            ...
            await axios.post("https://adminbackend.base2brand.com/api/B2Badmin/page/create", form);
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-md-12 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    placeholder="e.g., Low Conversion Rates"
                                                    value={item.challenge_des}
                                                    onChange={e => {
                                                        const items = [...formData.challengesSection.items];
                                                        items[idx].challenge_des = e.target.value;
                                                        setFormData((prev: any) => ({
                                                            ...prev,
                                                            challengesSection: { ...prev.challengesSection, items },
                                                        }));
                                            ...
                                            <div className="col-md-12 mb-3">
                                                ...
                                                <ReactQuill
                                                    theme="snow"
                                                    value={item.solution_des}
                                                    onChange={(value) => {
                                                        const items = [...formData.challengesSection.items];
                                                        items[idx].solution_des = value;
                                                        setFormData((prev: any) => ({
                                                            ...prev,
                                                            challengesSection: { ...prev.challengesSection, items },
                                                        }));
                                ...
                                <div key={idx} className="card mb-3">
                                    <div className="card-header d-flex justify-content-between align-items-center">
                                        ...
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                const faqs = formData.faqSection.filter((_: any, i: number) => i !== idx);
                                                setFormData((prev: any) => ({
                                                    ...prev,
                                                    faqSection: faqs,
                                                }));
                                            }}
                                            disabled={formData.faqSection.length <= 1}
                                        >
                                            Remove FAQ
                                        </button>
                                    </div>
                                        ...
                                        <div className="row">
                                            <div className="col-md-12 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    value={faq.question}
                                                    onChange={(e) => {
                                                        const faqs = [...formData.faqSection];
                                    ...
                                    setFormData((prev: any) => ({
                                        ...
                                        faqSection: [
                                            ...prev.faqSection,
                                            {
                                                question: "",
                                                answer: "",
                                                order: prev.faqSection.length + 1,
                                            },
                                        ],
                                    }))
                                }
